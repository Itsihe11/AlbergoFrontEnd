    ...
    this.http.post<any>('/api/utente/login', payload).subscribe({
      next: (res) => {
        ...
        if (res.ruolo === 'ADMIN') {
          ...
          else if (res.codicePrenotazione) {
            this.prenotazioniService.getPrenotazioneByCodice(res.codicePrenotazione).subscribe({
              next: (prenotazioneDB) => {
                localStorage.setItem('prenotazione_corrente', JSON.stringify(prenotazioneDB));
                this.router.navigate(['/clienti']);
              },
              error: () => {
                this.errore = 'Impossibile caricare i dettagli della prenotazione.';
              }
            });
          } 
          else {
            this.errore = 'Nessuna prenotazione associata a questo account.';
          }
      ...
      error: (err) => {
        ...
        this.errore = err.error?.error || err.error?.message || 'Credenziali non valide (Email o PIN errati).';
      }
    });
  }
}
